...
│   ├── title_layout.h      # Manual embeddable output for title screen
│   └── standalone/         # Original standalone exports with main() (archived, not built)
│       ├── title.c/.h
│       └── ...
└── adapters/               # Thin C++ adapters (compile-safe)
    ├── title_adapter.cpp   # Bridges title_layout.h to game systems
    └── title_adapter.h
```
...
5. Created thin adapter: `app/ui/adapters/title_adapter.{cpp,h}`
6. Moved original standalone exports to `app/ui/generated/standalone/` (not built)
...
- **Adapters not wired**: `title_adapter_render()` is not called from `ui_render_system`
...
3. Wire `app/ui/adapters/title_adapter.cpp` into CMakeLists.txt
4. Call `title_adapter_render()` from `ui_render_system` when `GamePhase::Title`
...
Adapters live in `app/ui/adapters/` and have this signature:

```cpp
void <screen>_adapter_init();                      // Call once
void <screen>_adapter_render(entt::registry& reg); // Call every frame
```

Adapters:
- ✅ Include generated layout header
...
2. **Archive standalone outputs** in `app/ui/generated/standalone/` (reference only)
