...
content/audio/            LoadMusicStream()              ┌──────────────┐
  2_drama.flac ───────────────────────────────────────▶  │ MusicContext  │◀───────────────────────  song_playback_system
                                                         ...
                                                         ┌──────────────────┐
                                                         │ entt::dispatcher │◀──────────────────  input/collision/ui/game_state
                                                         │ PlaySfxEvent     │  enqueue fire-and-forget events
                                                         │ PlayHapticEvent  │
                                                         └──────┬───────────┘
                                                                │ update<T>() drains each event type
                          wire_audio_haptic_dispatcher() ──────┼─────────────────────────────┐
                          connects dispatcher sinks             │                             │
                                                                ▼                             ▼
                                                           audio_system                  haptic_system
                                                           PlaySfxEvent → SFXBank        PlayHapticEvent → platform haptics
                                                           PlaySound()
```
...
`audio_system.cpp` now calls raylib audio functions and drains dispatcher events. `shapeshifter_lib` links `raylib`, so the system can remain in the library while `game_loop_init()` wires `PlaySfxEvent` to `audio_handle_play_sfx()` through `wire_audio_haptic_dispatcher()`. Do not reintroduce a separate singleton queue; gameplay code emits fire-and-forget dispatcher events instead.
...
## Change 6: audio_system.cpp — Dispatcher Event Drain

### Data
Reads `entt::dispatcher` from `reg.ctx()` and drains `PlaySfxEvent`.
`audio_handle_play_sfx()` reads `SFXBank` and calls raylib `PlaySound()` when the clip is loaded and the audio device is ready.

Haptics use the same dispatcher pattern: producers enqueue `PlayHapticEvent`, `haptic_system()` drains it, and `haptic_handle_play()` gates playback through settings before calling the platform haptics backend.

### File
**`app/systems/audio_system.cpp`**
...
### Dispatcher registration and draining

`game_loop_init()` creates `entt::dispatcher` in registry context, then calls `wire_audio_haptic_dispatcher(reg)`. That wiring connects:

- `PlaySfxEvent` → `audio_handle_play_sfx(reg, evt)`
- `PlayHapticEvent` → `haptic_handle_play(reg, evt)`

`audio_system(reg)` owns the post-render `disp.update<PlaySfxEvent>()` drain. `haptic_system(reg)` owns the matching `disp.update<PlayHapticEvent>()` drain. `game_loop_shutdown()` calls `unwire_audio_haptic_dispatcher(reg)` before clearing entities so scoped connections are released before registry teardown.

### Dependencies
Requires `entt::dispatcher` in `reg.ctx()` and `wire_audio_haptic_dispatcher(reg)` during initialization. Producers should use `disp.enqueue<PlaySfxEvent>({SFX::...})` or `disp.enqueue<PlayHapticEvent>({HapticEvent::...})`; they should not call raylib audio or platform haptics directly.
...
Phase 1 (parallel, no dependencies):
  ...
  ├── Change 6a: audio event payloads
  ├── Change 6b: audio/haptic dispatcher wiring
  ├── Change 6c: audio event drain
  └── Change 6d: haptic event drain
...
| `entt::dispatcher` audio/haptic queues | event payload sized | frame-local burst | small vectors | Producers enqueue SFX/haptic events; audio/haptic systems drain by event type | L1 for typical bursts |
