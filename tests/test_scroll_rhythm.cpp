...
TEST_CASE("scroll: non-rhythm entities use dt-based movement even in rhythm mode", "[scroll][rhythm]") {
    ...
    reg.emplace<Position>(particle, 100.0f, 200.0f);
    reg.emplace<Velocity>(particle, 10.0f, 20.0f);
    // No BeatInfo → dt-based

    scroll_system(reg, 0.5f);

    CHECK_THAT(reg.get<Position>(particle).x, Catch::Matchers::WithinAbs(105.0f, 0.01f));
    CHECK_THAT(reg.get<Position>(particle).y, Catch::Matchers::WithinAbs(210.0f, 0.01f));
}
