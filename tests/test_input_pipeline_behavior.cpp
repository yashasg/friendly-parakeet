...
//
// The pipeline: gesture_routing_system → hit_test_system → player_input_system
//
...
// Runs the three input-consuming systems in game-loop order for one sub-tick.
...
TEST_CASE("pipeline: swipe right produces lane change in same pipeline call — no latency",
    ...
    reg.ctx().get<EventQueue>().push_input(InputType::Swipe, 0.f, 0.f, Direction::Right);
    run_pipeline(reg);
...
TEST_CASE("pipeline: swipe left produces lane change in same pipeline call — no latency",
    ...
    reg.ctx().get<EventQueue>().push_input(InputType::Swipe, 0.f, 0.f, Direction::Left);
    run_pipeline(reg);
...
TEST_CASE("pipeline: swipe Up/Down has no lane effect",
    ...
    eq.push_input(InputType::Swipe, 0.f, 0.f, Direction::Up);
    eq.push_input(InputType::Swipe, 0.f, 0.f, Direction::Down);
    run_pipeline(reg);
...
TEST_CASE("pipeline: tap on active button triggers shape change in same pipeline call",
    ...
    reg.ctx().get<EventQueue>().push_input(InputType::Tap, 200.f, 200.f);
    run_pipeline(reg);
...
TEST_CASE("pipeline: tap on button inactive in current phase has no effect",
    ...
    reg.ctx().get<EventQueue>().push_input(InputType::Tap, 100.f, 100.f);
    run_pipeline(reg);

    // hit_test_system filtered it out (ActiveTag absent in Title) AND
...
TEST_CASE("pipeline: mixed swipe and tap both take effect within a single pipeline call",
    ...
    auto& eq = reg.ctx().get<EventQueue>();
    eq.push_input(InputType::Swipe, 0.f, 0.f, Direction::Right);  // lane +1
    eq.push_input(InputType::Tap,   200.f, 200.f);                 // shape Triangle
...
TEST_CASE("pipeline: swipe consumed after first sub-tick — second sub-tick does not reset lane.lerp_t (#213 behavior)",
    ...
    eq.push_input(InputType::Swipe, 0.f, 0.f, Direction::Right);
    run_pipeline(reg);
    CHECK(lane.target  == 2);
    ...
    // Sub-tick 2: input_system would have cleared the raw input queue at the
    // start of the frame; simulate that (no new raw inputs).
    eq.input_count = 0;
    run_pipeline(reg);
...
TEST_CASE("pipeline: tap consumed after first sub-tick — second sub-tick does not re-open shape window (#213 behavior)",
    ...
    eq.push_input(InputType::Tap, 100.f, 100.f);
    run_pipeline(reg);
    CHECK(sw.phase        == WindowPhase::MorphIn);
