...
TEST_CASE("title hitbox: tap at EXIT_TOP triggers Exit, not Confirm",
    ...
    eq.push_input(InputType::Tap, constants::SCREEN_W / 2.0f, EXIT_TOP);
    hit_test_system(reg);
...
TEST_CASE("title hitbox: tap clearly inside Confirm region triggers Confirm only",
    ...
    eq.push_input(InputType::Tap, constants::SCREEN_W / 2.0f, 400.0f);
    hit_test_system(reg);
...
TEST_CASE("title hitbox: tap inside Exit region triggers Exit only",
    ...
    eq.push_input(InputType::Tap, constants::SCREEN_W / 2.0f, EXIT_CENTER_Y);
    hit_test_system(reg);
