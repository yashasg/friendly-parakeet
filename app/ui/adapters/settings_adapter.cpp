...
using SettingsAdapter = RGuiAdapter<SettingsLayoutState,
                                    &SettingsLayout_Init,
...
void settings_adapter_render(entt::registry& reg) {
    ...
    // Dynamic toggle buttons with live labels
    {
        ...
        settings_adapter.state().HapticsTogglePressed =
            GuiButton(offset_rect(settings_adapter.state().Anchor01, 152, 720, 416, 100), haptics_label);
    }
    {
        ...
        settings_adapter.state().ReduceMotionTogglePressed =
            GuiButton(offset_rect(settings_adapter.state().Anchor01, 152, 880, 416, 100), motion_label);
    }

    // Dispatch actions
    if (st) {
        if (settings_adapter.state().AudioOffsetMinusPressed) {
        ...
        if (settings_adapter.state().AudioOffsetPlusPressed) {
            st->audio_offset_ms = static_cast<int16_t>(
                std::min(static_cast<int>(SettingsState::MAX_AUDIO_OFFSET_MS),
                         static_cast<int>(st->audio_offset_ms) + SettingsState::AUDIO_OFFSET_STEP_MS));
        }
        if (settings_adapter.state().HapticsTogglePressed) {
            st->haptics_enabled = !st->haptics_enabled;
        }
        if (settings_adapter.state().ReduceMotionTogglePressed) {
            st->reduce_motion = !st->reduce_motion;
        }
    }

    if (settings_adapter.state().CloseButtonPressed) {
