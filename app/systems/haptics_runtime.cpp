#include "haptics_runtime.h"

#include "../platform/ios/haptics_ios_bridge.h"
...
void trace_haptic_event(const char* prefix, HapticEvent event) {
    ...
    if (event_name.empty()) {
        std::fprintf(stderr, "%sUnknown\n", prefix);
        return;
    }
    std::fprintf(stderr, "%s%.*s\n", prefix, static_cast<int>(event_name.size()), event_name.data());
}
...
void trigger(HapticEvent event) noexcept {
    const char* prefix = "HAPTIC: ";
#if defined(PLATFORM_IOS)
    if (platform::ios::haptics_ios_available()) {
        platform::ios::haptics_ios_trigger(event);
        return;
    }
    prefix = "HAPTIC [iOS-stub]: ";
#endif
    trace_haptic_event(prefix, event);
}
