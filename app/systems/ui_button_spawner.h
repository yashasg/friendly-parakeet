...
//
// Layout constants match level_select_system.cpp exactly.
inline void spawn_level_select_buttons(entt::registry& reg) {
    ...
    // 3 difficulty buttons (positioned relative to first card;
    // hit_test / level_select can reposition based on selected_level)
    for (int d = 0; d < LevelSelectState::DIFFICULTY_COUNT; ++d) {
        float bx = DIFF_BTN_X0
                 + static_cast<float>(d) * (DIFF_BTN_W + DIFF_BTN_GAP)
                 + DIFF_BTN_W / 2.0f;
        float by = CARD_START_Y + DIFF_BTN_Y_OFF + DIFF_BTN_H / 2.0f;
