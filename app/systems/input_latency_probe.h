...
inline uint32_t input_latency_frames_since_last_input(const InputLatencyProbe& probe) {
    if (probe.last_input_frame > probe.frame_index) {
        return 0;
    }
    return static_cast<uint32_t>(probe.frame_index - probe.last_input_frame);
}
...
inline void input_latency_note_go_event_enqueued(entt::registry& reg, Direction dir) {
    ...
    probe->last_direction = dir;
    probe->last_input_to_go_enqueue_frames = input_latency_frames_since_last_input(*probe);
...
inline void input_latency_note_go_event_handled(entt::registry& reg, Direction dir) {
    ...
    probe->last_direction = dir;
    probe->last_input_to_go_handle_frames = input_latency_frames_since_last_input(*probe);
