...
### 2026-05-19 — Portable C rGuiLayout Integration Spec — REVISED

**Status:** SPEC REVISED — Coordinator feedback addressed; now aligns team correctly.

**Correction:** First draft recommended `app/ui/adapters/` as the primary file layout and function naming (e.g., `title_adapter_render()`). Coordinator found this contradicted the user's request. Revised spec to:
- **Primary contract:** Portable generated C/header (`.rgl` → generated `_layout.h`)
- **Implementation:** Screen controllers (not adapters), in `app/ui/screen_controllers/`
- **Function naming:** `render_<screen>_screen_ui()` or `<screen>_render_ui()` (not `_adapter_render`)
- **Legacy:** Mention `adapters/` only in migration section; it is transitional naming to be phased out

**Key revision outcomes:**
1. **Clarified architecture:** Generated portable C is upstream, primary contract. Screen controllers are downstream, game-specific integration.
2. **File layout revised:** `app/ui/screen_controllers/<screen>_screen_controller.cpp` (not adapters)
3. **Function examples:** `render_title_screen_ui(reg)` (not `title_adapter_render()`)
4. **Migration path:** Plan for migrating existing `adapters/` → `screen_controllers/`, not recommending adapters as the target
5. **Consistency:** Spec now unmistakably conveys that portable C is the contract, not the adapter pattern

**Why the confusion:** Initial spec used "adapter" terminology heavily in examples and file paths, which misdirected readers toward thinking adapters were the architectural boundary. Second revision elevates portable C/header to the primary role and relegates adapters to legacy naming in a migration section.

**Lesson:** When clarifying architecture, ensure examples and file paths reinforce the primary concept. Don't let implementation naming (adapters) overshadow the contract (portable C).

**Spec location:** `design-docs/rguilayout-portable-c-integration.md` (revised, 11 sections)  
**Decision updated:** `.squad/decisions/inbox/keyser-portable-rgl-integration.md` (approved, not draft)

### 2026-05-19 — Portable C rGuiLayout Integration Spec

**Status:** SPEC WRITTEN — Aligns team on integration boundary.

**Scope:** Clarified the conceptual frame for rguilayout integration from "adapter-layer implementation" to **"portable generated C as the integration boundary."**

**Spec location:** `design-docs/rguilayout-portable-c-integration.md` (17KB, 10 sections)

**Key outcomes:**
1. **Terminology shift:** Generated `.h` files are the contract (portable, reusable, upstream-owned). Adapters are game-specific glue (thin, ECS-aware, downstream).
2. **Non-goals clarified:** Strict prohibition on ECS widget mirrors, layout cache structs, copying rectangles, hand-editing generated files, compiling standalone exports.
3. **API shape documented:** `Init()` / `State` / `Render()` for all layouts. No update/cleanup unless custom resources are introduced.
4. **RAYGUI_IMPLEMENTATION ownership:** Single `raygui_impl.cpp` defines the raygui implementation; all adapters and generated headers safely include `raygui.h`.
5. **Event-consumer design:** UI effects (flashes, shakes) applied around layout render, not within generated code.
6. **File layout standardized:** `app/ui/generated/<screen>_layout.h` (portable), `app/ui/adapters/<screen>_adapter.cpp` (game wiring).
7. **Migration path:** For any existing code that rebuilds layout state or mirrors ECS components, documented correct pattern.

**Why this matters:** The current adapter implementations already follow portable C pattern correctly. This spec prevents future architectural drift and unifies terminology for the team.

**Decision written:** `.squad/decisions/inbox/keyser-portable-rgl-integration.md`
