...
## 2026-05-19 — Spec Terminology Consistency (rGuiLayout Portable C)

- When a spec has both a "target architecture" and a "legacy/migration" section, adapter terminology must be strictly confined to the migration section. Any stray references in event-consumer design, RAYGUI_IMPLEMENTATION comments, compiler warnings, testing, and open decisions will cause confusion about which pattern is the goal.
- Checklist for future spec reviews: scan for deprecated terms outside explicitly marked legacy/migration sections before finalising.


## 2026-05-20 — Patched Stale Adapter Wording in rGuiLayout Spec

- Replaced "Adapters include both and remain warning-free" with "Screen controllers include both and remain warning-free" in RAYGUI_IMPLEMENTATION Ownership section (line 291).
- Scanned migration section and confirmed adapter terminology remains confined to that legacy/current-naming context only.


## 2026-05-20 — Removed Stale Adapter References from screen_controller_base.h

- Updated line 2 comment from "Replaces adapter_base.h and end_screen_dispatch.h from the old adapters layer" to "Provides base class and helper utilities for RGui-based screen implementations."
- Removed post-migration stale reference while preserving intentional legacy naming in docs/migration sections.


## 2026-05-20 — Full Spec Correctness Pass (Kujan Rejection Resolved)

Resolved Kujan's documentation-only rejection of the rguilayout migration. Changes made:

**`design-docs/rguilayout-portable-c-integration.md`:**
- Section 3 code example: replaced fake `Dispatcher`/`GameCommand` API with actual `RGuiScreenController<>` template pattern and `reg.ctx().get<GameState>()` mutation pattern from real screen controllers.
- File Layout section: replaced "Legacy note" (adapters are current/future) with factual note that screen controllers are the current and only pattern.
- Migration Path section: completely rewrote as completed history ("Migration History: … Completed") rather than future steps. Removed all forward-looking adapter migration instructions. Correct Pattern updated to use `RGuiScreenController` template.
- Validation checklist: marked adapter-migration item as `[x]` complete with note.

**Generated header comment cleanup (safe comment-only edits):**
- `app/ui/generated/gameplay_hud_layout.h`: "This adapter provides" → "The screen controller provides"
- `app/ui/generated/level_select_screen_layout.h`: same pattern
- `app/ui/generated/settings_layout.h`: two occurrences — "The adapter renders them" and "handle toggle buttons in adapter" updated to screen controller wording.
- `app/ui/generated/standalone/README.md`: "Safe to include in adapters" → "Safe to include in screen controllers"

**Acceptance check:** `grep adapter|adapters|GameCommand|Dispatcher` across spec + generated + template + screen_controllers returns only two historical-context hits in the spec (migration history paragraph + completed checklist item). Zero misleading current/future adapter references remain.

**Lesson:** When a migration completes, spec sections describing it as future work must be rewritten as completed history immediately — not just patched at obvious spots. The code example section and the "correct pattern" subsection are easy to miss.
